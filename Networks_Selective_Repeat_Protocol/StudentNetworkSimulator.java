...
public class StudentNetworkSimulator extends NetworkSimulator
{
   ...
   // This is the constructor.  Don't touch!
    ...
    void sendPacketToB ()
    {
    	while (sendWindow.size() < WindowSize )
    	{
    		...
    		Packet aOutputPacket = new Packet(sequenceNumber,sequenceNumber, checkSumValue, message.getData());
    ...
    {
    	if ( !aSequenceInRange(ackNum, begA, endA) )
    	{
    		if (ackNum == begA-1 )
    		{
    			return true;
    		}
    		else if (begA == 0 && ackNum == LimitSeqNo -1)
			{
				return true;
			}
    	}
   ...
   // void resetTimerQueue(int val)
    ...
    protected void aInput(Packet packet)
    {
    	...
    	{
    		numOfAckPackets += 1;
    		if (!aSequenceInRange(packet.getAcknum(),begA,endA))
    		{
    			if(checkDuplicateAck(packet.getAcknum()))
    			{
    			...
    			{
    				System.out.println("A::Unexpected Ack received! This should not occur as the system does not reorder packets");
    			}
    		}
    		else 
    		{
    	...
    	if (packet.getChecksum() == calculateCheckSum(packet.getPayload() + packet.getSeqnum()) )
    	{
    		...
    		if ( aSequenceInRange (packet.getSeqnum(), begB, endB ) )
    		{
    			...
    			else if ( !isNacked )
    			{
    				...
    				Packet dupAckPacket = new Packet(acknum,acknum,checksum, message);
    		...
    		{
    			//System.out.println("B:: Old packet received with seqnum " + packet.getSeqnum() + " Dropping it" + a);
    			String message = "";
				int acknum;
				if (expectedSequenceNumberB == 0)
				{
